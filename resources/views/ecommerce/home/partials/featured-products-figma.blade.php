        ...
        <div class="col-12 col-sm-6 col-md-4 col-lg-3 mb-4 d-flex align-items-stretch">
          <div class="card shadow-sm rounded-3 h-100 position-relative">
            @php
              $firstImage = null;
              try {
                $firstImage = optional($product->images->first())->full_url_img;
              } catch (\Throwable $e) {
                $firstImage = null;
              }
              $imgSrc = $firstImage ?: ($product->url_thumbnail ?? asset('assets/cartzilla/images/og-image.jpg'));
            @endphp
            <div class="position-relative" style="height:267px; overflow:hidden;">
              <img src="{{ $imgSrc }}" class="card-img-top object-fit-cover w-100 h-100" alt="{{ $product->name }}">
            </div>
